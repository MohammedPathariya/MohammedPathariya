...
USER_NAME = os.environ['USER_NAME']  # 'MohammedPathariya'
QUERY_COUNT = {'user_getter': 0, 'follower_getter': 0, 'graph_repos_stars': 0, 'recursive_loc': 0, 'graph_commits': 0, 'loc_query': 0}


def training_since(start_date):
    """
    Returns time elapsed since a fixed start date (not a birthday)
...
def svg_overwrite(filename, training_data, commit_data, star_data, repo_data, contrib_data, follower_data, loc_data):
    tree = etree.parse(filename)
    root = tree.getroot()
    justify_format(root, 'training_data', training_data, 20)
...
if __name__ == '__main__':
    ...
    training_data, training_time = perf_counter(training_since, datetime.datetime(2020, 8, 1))
    formatter('training duration', training_time)
    ...
    for index in range(len(total_loc)):
        total_loc[index] = '{:,}'.format(total_loc[index]) if isinstance(total_loc[index], int) else total_loc[index]

    svg_overwrite('dark_mode.svg', training_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc)
    svg_overwrite('light_mode.svg', training_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc)
